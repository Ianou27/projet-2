...
describe('WaitingPlayerTwoComponent', () => {
    ...
    it('join() should close the dialogs on both side and make the second player join the room ', () => {
        ...
        const joinSpy = spyOn(component.chatService, 'joinRoom').and.callFake;
        component.join();
        expect(closeSpy).toHaveBeenCalled();
        expect(joinSpy).toHaveBeenCalled();
    });

    it('refused() should open the dialog', () => {
        // eslint-disable-next-line dot-notation
        const closeSpy = spyOn(component['multiplayerDialog'], 'open');
        component.refused();
        expect(closeSpy).toHaveBeenCalledWith(JoinPageComponent, { disableClose: true });
    });
});
